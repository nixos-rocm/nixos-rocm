...
# ROCm deps
, rocmSupport ? false, config ? null, runCommand ? null, lndir ? null, rocr ? null, rccl ? null
...
let
  ...
  rocmtoolkit_joined = if rocmSupport then runCommand "unsplit_rocmtoolkit" {} ''
    ...
    ln -s ${rocr} $out/hsa
    for i in ${hip} ${hipsparse} ${rocrand}/hiprand ${rocrand}/rocrand ${rocfft} ${rocblas} ${miopen-hip} ${miopengemm} ${rccl} ${hipcub} ${rocprim} ${rocr} ${binutils.bintools}; do
