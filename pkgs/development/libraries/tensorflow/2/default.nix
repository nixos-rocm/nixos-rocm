...
# ROCm deps
, rocmSupport ? false, config ? null, runCommand ? null, lndir ? null, rocm-runtime ? null, rccl ? null
, hip ? null, rocprim ? null, hipcub ? null, rocsparse ? null, hipsparse ? null, rocblas ? null
, miopengemm ? null, miopen-hip ? null, rocrand ? null, rocfft ? null
# MKL Support
...
let
  ...
  rocmtoolkit_joined = if rocmSupport then runCommand "unsplit_rocmtoolkit" {} ''
    ...
    for i in ${hip} ${hipsparse} ${rocrand}/hiprand ${rocrand}/rocrand ${rocfft} ${rocblas} ${miopen-hip} ${miopengemm} ${rccl} ${hipcub} ${rocprim} ${rocm-runtime} ${binutils.bintools}; do
...
in buildPythonPackage {
  ...
  nativeBuildInputs = lib.optional (cudaSupport || rocmSupport) addOpenGLRunpath;
