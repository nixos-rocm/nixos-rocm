...
let
  ...
  commonStructuredConfig = import (<nixpkgs> + /pkgs/os-specific/linux/kernel/common-config.nix) {
  ...
  configfile = stdenv.mkDerivation {
    ...
    buildPhase = ''
      ...
      DEBUG=1 ARCH=$kernelArch KERNEL_CONFIG="$buildRoot/kernel-config" AUTO_MODULES=$autoModules DEFCONFIG=$defconfig \
